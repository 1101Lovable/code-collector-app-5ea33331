-- 1) Common helper: updated_at trigger function
...
-- 2) Tables (create first, then RLS/policies)
-- profiles
...
-- public cultural data (read-only, public view)
...
-- 4) Policies (drop if exist, then create) - dependencies now exist
-- profiles
...
CREATE POLICY "Users can view family groups they belong to" ON public.family_groups FOR SELECT USING (
  EXISTS (
...
CREATE POLICY "Family creators can update their groups" ON public.family_groups FOR UPDATE USING (created_by = auth.uid());
CREATE POLICY "Users can create family groups" ON public.family_groups FOR INSERT WITH CHECK (created_by = auth.uid());
CREATE POLICY "Family creators can delete their groups" ON public.family_groups FOR DELETE USING (created_by = auth.uid());
...
CREATE POLICY "Users can view family members of their groups" ON public.family_members FOR SELECT USING (
  EXISTS (
...
CREATE POLICY "Family heads can manage members" ON public.family_members FOR ALL USING (
  EXISTS (
...
CREATE POLICY "Users can insert themselves as members" ON public.family_members FOR INSERT WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "Users can view their own schedules" ON public.schedules FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Users can view family schedules" ON public.schedules FOR SELECT USING (
...
CREATE POLICY "Users can create their own schedules" ON public.schedules FOR INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY "Users can update their own schedules" ON public.schedules FOR UPDATE USING (user_id = auth.uid());
CREATE POLICY "Users can delete their own schedules" ON public.schedules FOR DELETE USING (user_id = auth.uid());

-- mood_records (family visibility via join)
...
CREATE POLICY "Users can view their own mood records" ON public.mood_records FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Users can view family members' mood records" ON public.mood_records FOR SELECT USING (
  EXISTS (
...
CREATE POLICY "Users can create their own mood records" ON public.mood_records FOR INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY "Users can update their own mood records" ON public.mood_records FOR UPDATE USING (user_id = auth.uid());
CREATE POLICY "Users can delete their own mood records" ON public.mood_records FOR DELETE USING (user_id = auth.uid());

-- cultural tables are publicly readable
DROP POLICY IF EXISTS "Cultural events are viewable by everyone" ON public.cultural_events;
CREATE POLICY "Cultural events are viewable by everyone" ON public.cultural_events FOR SELECT USING (true);
DROP POLICY IF EXISTS "Cultural spaces are viewable by everyone" ON public.cultural_spaces;
CREATE POLICY "Cultural spaces are viewable by everyone" ON public.cultural_spaces FOR SELECT USING (true);
...
CREATE INDEX IF NOT EXISTS idx_cultural_events_district ON public.cultural_events(district);
CREATE INDEX IF NOT EXISTS idx_cultural_events_dates ON public.cultural_events(start_date, end_date);
CREATE INDEX IF NOT EXISTS idx_cultural_spaces_district ON public.cultural_spaces(district);