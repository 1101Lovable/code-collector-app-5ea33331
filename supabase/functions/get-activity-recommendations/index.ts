...
serve(async (req) => {
  ...
  try {
    ...
    const eventsContext = events && events.length > 0
      ? events.map(e => `- ${e.title} (${e.place || '장소 미정'}): ${e.program_description || e.event_type || '문화 행사'}`).join('\n')
      : '현재 등록된 행사가 없습니다.';

    const spacesContext = spaces && spaces.length > 0
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: '당신은 어르신들을 위한 친절하고 배려심 깊은 활동 추천 도우미입니다. 건강하고 즐거운 활동을 추천해주세요.'
          },
          {
            role: 'user',
            content: prompt
